...
const Calculators = () => {
  ...
  const textsToTranslate = [
    ...
    "Estimate your premium costs and coverage details",
    "State",
    "District",
    "Crop",
    "Area (hectares)",
    ...
    "Please enter a valid area value greater than 0",
    "Crop parameters not found. Please select a valid crop.",
  ];
  ...
  const validateInputs = () => {
    if (!state) {
      setError(getTranslatedText("Please select a state"));
      return false;
    }

    if (!district) {
      setError(getTranslatedText("Please select a district"));
      return false;
    }

    if (!crop) {
      setError(getTranslatedText("Please select a crop"));
      return false;
    }

    const areaValue = parseFloat(area);
    if (isNaN(areaValue) || areaValue <= 0) {
      setError(getTranslatedText("Please enter a valid area value greater than 0"));
      return false;
    }
  ...
  const calculatePremium = () => {
    ...
    setTimeout(() => {
      ...
      if (selectedCropParams) {
        ...
        setError(getTranslatedText("Crop parameters not found. Please select a valid crop."));
      }
